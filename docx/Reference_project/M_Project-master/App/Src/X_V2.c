...
{
  uint16_t i = 0, j = 0, len = 0; static uint8_t cmd[MMCL_LEN] = {0};
...
{
  static uint8_t cmd[16] = {0}; uint16_t v = 0;
...
void X_V2_MMCL_Trig_Encoder_Cal(uint8_t addr)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 4; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_En_Control(uint8_t addr, bool state, bool snF)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 6; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
{
  uint8_t j = 0; static uint8_t cmd[16] = {0};
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 9; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
{
  uint8_t j = 0; static uint8_t cmd[16] = {0}; uint16_t v = 0;
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 11; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Bypass_Pos_LV_Control(uint8_t addr, uint8_t dir, float vel, float pos, uint8_t raf, bool snF)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 12; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Bypass_Pos_LV_LC_Control(uint8_t addr, uint8_t dir, float vel, float pos, uint8_t raf, bool snF, uint16_t maxCur)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 14; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Traj_Pos_Control(uint8_t addr, uint8_t dir, uint16_t acc, uint16_t dec, float vel, float pos, uint8_t raf, bool snF)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 16; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Traj_Pos_LC_Control(uint8_t addr, uint8_t dir, uint16_t acc, uint16_t dec, float vel, float pos, uint8_t raf, bool snF, uint16_t maxCur)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 18; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Stop_Now(uint8_t addr, bool snF)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 5; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 20; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Origin_Read_SL_RP(uint8_t addr)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 3; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Origin_Modify_SL_RP(uint8_t addr, bool svF, uint16_t sl_rp)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 7; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Auto_Return_Sys_Params_Timed(uint8_t addr, SysParams_t s, uint16_t time_ms)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < i; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
{
  uint8_t j = 0; static uint8_t cmd[16] = {0}; uint8_t MotType = 0;
...
{
  uint8_t j = 0; static uint8_t cmd[32] = {0};
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 21; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
void X_V2_MMCL_Modify_DMX512_Params(uint8_t addr, bool svF, uint16_t tch, uint8_t nch, uint8_t mode, uint16_t vel, uint16_t acc, uint16_t vel_step, uint32_t pos_step)
{
  ...
  // 加载当前命令到多电机命令中
  for(j=0; j < 19; j++) { if (MMCL_count < MMCL_LEN) { MMCL_cmd[MMCL_count] = cmd[j]; ++MMCL_count; } }
}
...
/**
  ...
  * @param    addr  ：电机地址
  */
void X_V2_Read_Version(uint8_t addr)
{
